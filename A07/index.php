...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['requesterID']) && isset($_POST['requesteeID']) && isset($_POST['status'])) {
    ...
    if (isset($_POST['delete_friendID'])) {
        $deleteFriendID = $_POST['delete_friendID'];

        $deleteQuery = "DELETE FROM friends WHERE friendID = ?";
        if ($stmt = mysqli_prepare($conn, $deleteQuery)) {
            mysqli_stmt_bind_param($stmt, "i", $deleteFriendID);

            if (mysqli_stmt_execute($stmt)) {
                echo "<div id='successMessage' class='alert alert-success text-center'>Friend request deleted successfully!</div>";
            } else {
                echo "<div id='errorMessage' class='alert alert-danger text-center'>Error deleting friend request: " . mysqli_stmt_error($stmt) . "</div>";
            }
                        ...
                        <div class="col-lg-4 col-md-6 col-12 mb-4">
                            <div class="card">
                                <div class="card-body text-center position-relative">
                                    ...
                                    <form method="POST" action="" class="btnDelete d-flex justify-content-center align-items-center">
                                    <input type="hidden" name="delete_friendID" value="<?php echo htmlspecialchars($row['friendID']); ?>">
                                    <button type="submit" class="btn btn-submit">Delete</button>
                                    </form>

                                    <form method="GET" action="edit.php" class="btnEdit d-flex justify-content-center align-items-center">
                                    <input type="hidden" name="friendID" value="<?php echo htmlspecialchars($row['friendID']); ?>" />
